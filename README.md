...
브라우저 탭을 닫으면 **서버가 자동으로 종료**됩니다 (약 15초 후).
즉시 종료하려면 터미널 창에서 `Ctrl+C` 또는 창을 닫으세요.

### 동작 방식

```
더블클릭
  → reports/ 스캔 → manifest.json 생성
  → 로컬 서버 시작 (localhost:8765)
  → 브라우저 자동 오픈

브라우저 탭 열려있는 동안
  → 3초마다 heartbeat 전송 → 서버 유지

브라우저 탭 닫힘
  → heartbeat 중단 → 15초 후 서버 자동 종료
```

### 새 주차 리포트 추가 후

`view_reports.command`를 다시 실행하면 새 주차가 자동으로 감지됩니다.
...
├── reports/
│   ├── index.html          ← 뷰어 UI
│   ├── server.py           ← 로컬 서버 (heartbeat + manifest 생성)
│   ├── marked.min.js       ← 마크다운 렌더러 (오프라인 동작)
│   ├── manifest.json       ← 주차·파일 목록 (자동 생성)
│   └── 2026-W13/
│       ├── _리서치.md
│       ├── 시장조사.md
│       ├── PEST분석.md
